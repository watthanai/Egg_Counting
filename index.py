    ...
    for i in range(len(contours)):
        ...
        if len(contour) >= 5:
            ...
            if (radius <= int(radius_max) and radius >= int(radius_min)):
                ...
                # tx1 = axis[0].astype(int)
                # tx2 = axis[1].astype(int)
                # ax1 = np.isnan(int(round(axis[0])))
                # ax2 = np.isnan(int(round(axis[1])))

                tx1 = "{:.0f}".format(axis[0])
                tx2 = "{:.0f}".format(axis[1])
                # ax1 = np.isnan(tx1)
                # ax2 = np.isnan(tx2)
                if tx1=='nan':
                    tx1 = 0
                ...
                print("axis",axis,angle,tx1,tx2,type(tx1),type(tx2),ax1,ax2,)
